...
\#2 If you need date functions like time(), date(), or now() you have the activate the Dates package.
...
\#6 The structure of the Address datatype. The constructor `Address(address_type, address)` allows the user to only specify the AddressType and the address. The fields `id` and `created` are generated by Julia code.
